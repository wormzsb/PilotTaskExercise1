...
int iDirection;
int rtn;//程序返回值
HINSTANCE gHinstance;//全局句柄
float acce = 0;  //目标加速度
int g_nThreadExitCount = 0;             //当前运行的线程数
DWORD dwInputThreadID = 0;              //瞄准器输入线程ID      
double alpha,omiga,a,b,r,Rx,Ry,fai,AngleSpeed;
float r1,r2;
short m_TrackState;
POINT A,C;
double v;
...
struct TaskSetting2   m_Setting;        //任务2设置参数
...
const char Insturction1[] = "    屏幕上的白色“⊥”是目标，黄色“⊥”是追踪环。您的任务\
...
POINT m_IndicatorPoint;                                  //时间保持开始指示器坐标
BOOL m_bHoldStart;                                       //时间保持开始标志
BOOL m_bShowSign;                                        //时间保持开始指示器显示标志
BOOL m_bShowTime;

char FeedBack[30];                                       //呈现反馈字符串

#define D3DFVF_CUSTOMVERTEX (D3DFVF_XYZRHW|D3DFVF_DIFFUSE)

const double m_MoveInt = 0.04;
double m_SampleInt;                                      //采样间隔40ms

UINT m_PointNum;
UINT m_MemNum;
unsigned long *m_HoldStartTime = NULL;                   //存放时间保持开始时间的数组
unsigned long *m_HoldSureTime = NULL;                    //存放时间保持确认时间的数组 
int *m_HoldTimeOrder = NULL;                             //存放时间保持数值的数组
float *m_HoldTime = NULL;                                  //存放保持时间的数组
...
double dfMinus, dfFreq, dfTim, dfTotal, dfTotalMove; 
                     ...
                     int       nCmdShow)
{
    ...
    //创建窗口
    hWnd = CreateWindow( "ClassName", "Task2", 
