...
BOOL t3::m_bShowFeedback = FALSE;//显示反馈
float t3::acce = 0;	//目标加速度
int t3::iDirection = 0;  //顺逆时针
int t3::g_nThreadExitCount = 0;                               //当前运行的线程数
DWORD t3::dwInputThreadID = 0;                                //瞄准器输入线程ID 

double t3::alpha,t3::omiga,t3::v,t3::a,t3::b,t3::r,t3::r1,t3::r2,t3::Rx,t3::Ry,t3::fai,t3::AngleSpeed,t3::inc_v;
double t3::m_PostPointX,t3::m_PostPointY;                         //当前瞄准器坐标
int t3::JoyX,t3::JoyY;                                            //当前操纵杆输入值  
struct TaskSetting3   t3::m_Setting;                          //任务3设置参数
struct HardSetting   t3::m_HardSetting;                       //硬件设置参数
struct PartInfo   t3::m_PartInfo;                             //被试者信息
short t3::m_TrialType;                                        //测试类型
int t3::m_TrialTime;                                          //测试时间
int t3::m_TrialTimes;                                         //测试次数
int t3::iJoyMoveDirection;//手柄移动正向反向
int t3::x_resolution=1024;			//屏幕分辨率
int t3::y_resolution=768;             //屏幕分辨率
int t3::rec_x_begin;
int t3::rec_y_begin;
int t3::rec_x_end;
int t3::rec_y_end;
const float t3::FontScale = (float)(t3::x_resolution+t3::y_resolution)/1400.0;       //字体随屏幕分辨率的放缩尺度

const char t3::Insturction11[] = "    欢迎进入目标\
跟踪作业下的突发事件反应任务！在下面的测试中，你需要用右手操纵控制\
杆来完成目标跟踪任务。在跟踪的同时，你需要监测是否有飞机的图形出现，\
一旦出现，则用左手按键做出反应。";
const char t3::Insturction12[] = "    欢迎进入目标跟踪作业下的突发事件反应任务！在下面的测试中，\
你需要用右手操纵控制杆来完成字符编码作业。屏幕正上方会显示编码规则，\
当屏幕中央出现图形的时候，你需要按照这个规则来按击相应字母。在进行编\
码的同时，你需要监测是否有飞机的图形出现，一旦出现，则用左手按键作\
出反应。";
const char t3::Insturction13[] = "    欢迎进入目标跟踪作业下的突发事件反应任务！\
在下面的测试中，你需要用右手完成数字编\
码任务，同时监测是否有飞机的图形出现，则用左手按键作出反应，如果出\
现的是直升飞机的图形，则不需要按键。"; 
const char t3::Insturction2[] = "练习结束，按任意键进入正式实验。";
const char t3::Insturction3[] = "该任务完成，按任意键继续！";
const char t3::FeedBack[2][10] = {"错误","正确"};

LPDIRECT3D9             t3::g_pD3D       = NULL;             //directx3d对象
LPDIRECT3DDEVICE9       t3::g_pd3dDevice = NULL;             //directx设备对象
LPDIRECT3DTEXTURE9      t3::g_pTextureInst  = NULL;          //纹理对象
LPDIRECT3DTEXTURE9      t3::g_pTexture0  = NULL;             //纹理对象
LPDIRECT3DTEXTURE9      t3::g_pTexture1  = NULL;             //纹理对象
LPDIRECT3DTEXTURE9      t3::g_pTexture2  = NULL;             //纹理对象
LPDIRECT3DTEXTURE9      t3::g_pTexture3[2]  = {NULL};        //纹理对象
LPD3DXSPRITE            t3::g_pSprite  = NULL;               //精灵对象
LPD3DXFONT              t3::g_pFont      = 0;                //字体对象
LPD3DXFONT              t3::g_pFont1     = 0;                //字体对象
LPDIRECT3DVERTEXBUFFER9      t3::m_pVertexBuffer  = NULL;    //顶点对象

D3DXMATRIX t3::m_Transform;                                  //坐标变换矩阵
float t3::TimeInt;                                           //时间间隔 
short t3::m_EventNo;                                         //突发事件序号
short t3::m_EventCount;                                      //突发事件总数
int t3::m_RecordNo;                                          //突发事件记录序号
short t3::m_EventKeyNum;                                     //突发事件按键数 
short t3::m_SureDownNum;
BOOL t3::m_bDisplayReady;                                    //渲染设备就绪标志

BOOL t3::m_bEventStart;                                      //突发事件开始标志
BOOL t3::m_bEventReactTime = FALSE;									//是否进入等待反应时间标志
int t3::iPlanePos;


const double t3::m_MoveInt = 0.04;                           //采样间隔40ms
double t3::m_SampleInt;
BOOL t3::m_bEventAcc;                                        //按键确认正确标志

UINT t3::m_PointNum;
UINT t3::m_MemNum, t3::m_MemEvent;

/*short *t3::m_EventType = NULL;                               //存放突发事件类型的数组
int *t3::m_EventOrder = NULL;                                //存放突发事件类型序列的数组
int *t3::m_EventInterval = NULL;                             //存放突发事件开始间隔的数组
unsigned long *t3::m_EventStartTime = NULL;                  //存放突发事件开始时间的数组 
unsigned long *t3::m_EventSureTime = NULL;                   //存放突发事件确认时间的数组
short *t3::m_SureButtonNo = NULL;                            //存放突发事件确认键类型的数组
BOOL *t3::m_bHit = NULL;                                     //存放击中状态的数组
SPOINT *t3::m_ObjPoint = NULL;                               //存放目标坐标的数组
SPOINT *t3::m_PostPoint = NULL;                              //存放瞄准器坐标的数组
float *t3::m_Distance = NULL;                                //存放目标和瞄准器距离的数组
unsigned long *t3::m_PointTime = NULL;                       //存放采样时间的数组
POINT *t3::m_EventPoint = NULL;                              //存放突发事件显示坐标的数组
float *t3::m_ObjSpeedX = NULL;                               //存放目标速度的数组
float *t3::m_ObjSpeedY = NULL;                               //存放目标速度的数组
float *t3::m_PostSpeedX = NULL;                              //存放瞄准器速度的数组
float *t3::m_PostSpeedY = NULL;                              //存放瞄准器速度的数组*/

vector<short> t3::m_EventType;                               //存放突发事件类型的数组
int t3::m_EventOrder = NULL;                                //存放突发事件类型序列的数组
int t3::m_EventInterval = NULL;                             //存放突发事件开始间隔的数组
unsigned long t3::m_EventStartTime = NULL;                  //存放突发事件开始时间的数组 
unsigned long t3::m_EventSureTime = NULL;                   //存放突发事件确认时间的数组
short t3::m_SureButtonNo = NULL;                            //存放突发事件确认键类型的数组
BOOL t3::m_bHit = NULL;                                     //存放击中状态的数组
SPOINT t3::m_ObjPoint;                               //存放目标坐标的数组
SPOINT t3::m_PostPoint;                              //存放瞄准器坐标的数组
float t3::m_Distance = NULL;                                //存放目标和瞄准器距离的数组
vector<unsigned long> t3::m_PointTime;                       //存放采样时间的数组
vector<POINT> t3::m_EventPoint;                              //存放突发事件显示坐标的数组
float t3::m_ObjSpeedX = NULL;                               //存放目标速度的数组
float t3::m_ObjSpeedY = NULL;                               //存放目标速度的数组
float t3::m_PostSpeedX = NULL;                              //存放瞄准器速度的数组
float t3::m_PostSpeedY = NULL;                              //存放瞄准器速度的数组
	...
	std::string winClassName, std::string winName)
{
	...
	_hWnd = hWnd = CreateWindow(std::to_string(nCmdShow).c_str(), /*std::to_string(nCmdShow).c_str()*/"实验3-1",
