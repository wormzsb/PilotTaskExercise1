...
BOOL m_bCubeNum[4];                              //三维图形方块数标志

BOOL m_bAcc;                                     //三维图形记忆正确性标志
double alpha,omiga,v,a,b,r,Rx,Ry,fai,AngleSpeed,inc_v;
double m_PostPointX,m_PostPointY;                //当前瞄准器坐标
int JoyX,JoyY;                                   //当前操纵杆输入值  
struct TaskSetting6   m_Setting;                 //任务6设置参数
struct HardSetting   m_HardSetting;              //硬件设置参数
struct PartInfo   m_PartInfo;                    //被试者信息
short m_TrialType;                               //测试类型
int m_TrialTime;                                 //测试时间
int m_TrialTimes;                                //测试时间
int iJoyMoveDirection;//手柄运动方向正向反向
...
LPDIRECT3DTEXTURE9      g_pTexture3[12]  = {NULL};         //纹理对象
...
LPD3DXFONT              g_pFont2     = 0;                  //字体对象
LPDIRECT3DVERTEXBUFFER9      m_pVertexBuffer  = NULL;

D3DXMATRIX m_Transform;                          //坐标变换矩阵
float TimeInt;
short m_SignNo;                                  //三维图形记忆序号
short m_SignOrderNo1, m_SignOrderNo2;            //三维图形记忆系列序号
short m_SignTrialNo;                             //三维图形记忆次数号
short m_SignGroupNo;                             //三维图形记忆组号
short m_SignCount;                               //三维图形记忆数目
BOOL m_bShowMem;                                 //待记忆三维图形文件显示标志
BOOL m_bLoadSign;                                //三维图形文件加载标志
...
BOOL m_bHideSign = FALSE;						  //在等待时间隐藏三维图形
BOOL m_bRememStart;                               //三维图形记忆开始标志      
BOOL m_bSignStart;                                //三维图形显示标志
short m_SureDownNum;
int m_SignType[12];                               //三维图形类型
int m_SignOrder1[6];                              //三维图形序列
int m_SignOrder2[6];                              //三维图形序列
unsigned long m_SignStartTime[12];                //三维图形记忆开始时间
unsigned long m_SignSureTime[12];                 //三维图形记忆确认时间
char m_LMemName[30];                              //存放待记忆三维图形名称的数组
char m_RMemName[12][30];                          //存放记忆三维图形名称的数组
short m_LMemAngle;                                //存放待记忆三维图形旋转角度的数组
short m_RMemAngle[12];                            //存放记忆三维图形旋转角度的数组
short m_MemAngleDiff[12];                         //存放两边三维图形旋转角度差的数组
short m_NoSame[12];                               //存放三维图形相同性的数组
short m_SureButtonNo[12];                         //存放确认按键的数组
short m_ButtonDownNum;
...
RECT strurect,lerect,rerect,erect,txtrect,coderect,charrect[8];
D3DRECT SignRect;
...
//Timer
LARGE_INTEGER litmp; 
LONGLONG QPart1,QPart2,QPart3,QPart4;
double dfMinus, dfFreq, dfTim, dfTotalPause, dfTotalMove, dfTotalSign; 
                     ...
                     int       nCmdShow)
{
	...
	hWnd = CreateWindow( "ClassName", "Task6", 
